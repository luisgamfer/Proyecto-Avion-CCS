...
static int Cmd_traza(int argc, char *argv[])
{
    ...
    else
    {
        ...
        {
            UARTprintf("Traza encendida\r\n");
            traza = 1;

            xQueueOverwrite(cola_comandos,&traza);
        }
        else if (0==strncmp( argv[1], "off",3))
        {
            UARTprintf("Traza apagada\r\n");
...
// ==============================================================================
// Implementa el comando "RADIO"
// ==============================================================================
static int msg_radio(int argc, char *argv[])
{
    ...
    {
        //Si los parametros no son suficientes o son demasiados, muestro la ayuda
        UARTprintf(" radio mensaje\r\n");
    }
    else
    {

       // Lee los argumentos introducidos como mensaje, los copia al parametro de mensaje y los envia por la cola de mensajes radio
        for(i; i<argc && caracteres < 40; i++){

            while(argv[i][j] != NULL && caracteres < 40){

                mensaje_radio.caracteres[caracteres] = argv[i][j];
                caracteres++;
                j++;
            ...
            if((caracteres<40)){
                ...
                mensaje_radio.caracteres[caracteres] = ' ';
                caracteres++;
       ...
       xQueueOverwrite(cola_radio,mensaje_radio.caracteres);

       UARTprintf("Mensaje enviado\r\n\n");
...
tCmdLineEntry g_psCmdTable[] =
{
    ...
    { "traza",    Cmd_traza,     "      : Activa/desactiva el modo traza"},
    {"radio",     msg_radio,     "      : Envia un mensaje radio (40 caracteres)"},
#if ( configUSE_TRACE_FACILITY == 1 )
