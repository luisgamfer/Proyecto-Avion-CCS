...
//Esta función obtiene el campo "tipo mensaje" de la trama
uint8_t decode_message_type(uint8_t * buffer)
{
	return buffer[0];
}

//Esta funciï¿½n extrae el parametro de la trama y comprueba que el tamaï¿½o sea correcto
//ptrtoparam es el puntero a la zona de memoria donde esta el parametro
//param_size es su tamaï¿½o
//param es una estructura por REFERENCIA
//payload es el tamaï¿½o de la estructura que se pasa por REFERENCIA
//Devuelve: Tamaï¿½o del parametro recibido o un valor negativo si hay error
...
//campo es un puntero a void que se pasa por REFERENCIA. Quedaria apuntando a la zona de memoria donde está el parametro
